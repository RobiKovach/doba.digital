(() => {
    ...
    function initSliders() {
        if (document.querySelector(".swiper")) {
            new swiper_core_Swiper(".keys__slider", {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                slidesPerView: 3,
    ...
    document.addEventListener("DOMContentLoaded", (() => {
        const allBanners = [ {
            containerSelector: ".banner-hero",
            imageSelector: ".banner-hero__img"
        }, {
    ...
    document.addEventListener("DOMContentLoaded", (() => {
        const videoWrappers = document.querySelectorAll(".slide-comments__video");
        videoWrappers.forEach((wrapper => {
            const video = wrapper.querySelector("video.circle-video");
            const button = wrapper.querySelector(".play-button");
            if (!video || !button) return;
            const togglePlay = () => {
                document.querySelectorAll(".slide-comments__video .circle-video").forEach((v => {
                    if (v !== video) {
                        v.pause();
                        v.currentTime = 0;
                        const otherBtn = v.closest(".slide-comments__video")?.querySelector(".play-button");
